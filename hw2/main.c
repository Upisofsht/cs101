...
void print_emp_record (emp_record_t emp_record_t_i) {
    printf("請輸入要新增操作人員 ID(1-99) :");
    scanf("%d",&emp_record_t_i.emp_id);
    printf("請輸入要新增操作人員 Name :");
    scanf("%63s",emp_record_t_i.emp_name);
    printf("請輸入要新增操作人員 Salary :");
    scanf("%d",&emp_record_t_i.emp_salary);
    FILE* teempfp=fopen("operator_id.bin","ab");
    fwrite(&emp_record_t_i,sizeof(emp_record_t_i),1,teempfp);
    fclose(teempfp);
}
...
int main() {
    ...
    scanf("%d",&userid);
    if (userid==0) {
        print_emp_record(emp_record_t_i);
        printf("輸入完成");
    } else {
        fp=fopen(s1,"w+");
        write_id(userid);
